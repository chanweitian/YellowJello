			...
			while (rs.next()) {
				...
				for (int i = 0; i < b_array.length; i++) {
					...
					if (z_array.length != 0) {
						...
						new_b_value = new_b_value.substring(0,
								new_b_value.length() - 1);
				...
				String site_def_building_name = rs
						.getString("site_def_building_name");
				...
				if (z_array.length != 0) {
					...
					bName_array = ArrayUtils.remove(bName_array,
							buildingArrayPosition);
				}
				...
				for (int i = 0; i < b_array.length; i++) {
					new_site_def_activity = new_site_def_activity + b_array[i]
							+ "^";
					new_site_def_building_name = new_site_def_building_name
							+ bName_array[i] + "*";
				}
				new_site_def_activity = new_site_def_activity.substring(0,
						new_site_def_activity.length() - 1);
				new_site_def_building_name = new_site_def_building_name
						.substring(0, new_site_def_building_name.length() - 1);

				String values = "site_def_info = \'" + new_site_def_info
				...
				// update record
				SQLManager
						.updateRecords("site_definition", values, whereUpdate);
			}
